#include "rbms.h"
#include "mbed.h"
rbms::rbms(RawCAN &can,bool motor_type,int motor_num)
...
void rbms::spd_control(int* set_speed,int* motor){//速度制御用関数
    ...
    while(1){
        for(int id=0;id<_motor_num;id++){
            if(rbms_can_msg.id==0x201+id){//esc idごとに受信データ割り振り
                CANMessage msg=rbms_can_msg;
                rbms_read(msg,&rotation[id],&speed[id]);//data変換
                if(set_speed[id]>=500)set_speed[id]=500;//速度上限500rpm
                if(_motor_type){
                    motor[id] = (int)pid(tm[id].read(),speed[id]/19,set_speed[id],&delta_rpm_pre[id],&ie[id]);
                }else{
                    motor[id] = (int)pid(tm[id].read(),speed[id]/36,set_speed[id],&delta_rpm_pre[id],&ie[id],19,8);
                }
